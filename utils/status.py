...
class ProgressBar:
    ...
    def prog(self, i: int, max_iter: int, epoch: Union[int, str],
        ...
        if i:  # not (i + 1) % 10 or (i + 1) == max_iter:
            ...
            progress_bar = ('█' * int(25 * progress)) + ('┈' * (25 - int(25 * progress)))
            if isinstance(task_number, int):
                print('\r[ {} ] T{:02d} | e{:03d}: |{}| {} ep/h | loss: {:.2f} |'.format(
                    datetime.now().strftime("%m-%d | %H:%M"),
                    task_number + 1,
                    epoch + 1,
                    progress_bar,
                ...
                print('\r[ {} ] T {} | e{:03d}: |{}| {} ep/h | loss: {:.2f} |'.format(
                    datetime.now().strftime("%m-%d | %H:%M"),
                    task_number,
                    epoch + 1,
                    progress_bar,
