...
class MTL_Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        data = self.load_video(sample, self.subset)
        label = self.label_dict.get(sample).get('final_score')

        if self.args['usingDD']:
            diff = self.label_dict.get(sample).get('difficulty')
        else:
            diff = 1

        if self.subset == 'test':
            return data, np.array([label, diff])
        else:
            return data, np.array([label, diff]), data
...
def dataset_split(args, num_total_tasks, debug=False, fewshot=False, num_samples_per_task=20):
    ...
    for sample in train_samples:
        train_labels.append(label_dict.get(sample)['final_score'])

    test_labels = []
    for sample in test_samples:
        test_labels.append(label_dict.get(sample)['final_score'])
    ...
    def split_task(samples, subset='training'):
        ...
        def boundary_split(num_task):
            ...
            for sample in samples:
                label = label_dict.get(sample)['final_score']
            ...
            def key_func(sample):
                return label_dict.get(sample)['final_score']
...
class ClassMTL(ContinualDataset):
    NAME = 'class-mtl'
    SETTING = 'class-il'
    ...
    def __init__(self, args):
        super(ClassMTL, self).__init__(args)
    ...
    def get_joint_loader(self, subset='train'):
        ...
        #     print(all_labels.get(sample)['final_score'])
    ...
    def get_data_loader(self, transform, samples, subset='train'):
        dataset = MTL_Dataset(self.args.dataset_args, transform=transform,
